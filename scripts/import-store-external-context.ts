...
import { importStoreExternalContextSnapshot } from "../src/import-store-external-context-script.js";
...
function printUsage(): void {
  console.log(
    [
      "Usage:",
      "  node --import tsx scripts/import-store-external-context.ts --input data/store-external-context/yingbin-2026-04-18.json [--input another.json]",
    ].join("\n"),
  );
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const inputPath of options.inputPaths) {
      await importStoreExternalContextSnapshot({
        store,
        filePath: inputPath,
