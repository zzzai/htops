...
export type CustomerProfileUtteranceSample = {
  id: string;
  category: string;
  action: "profile";
  label: string;
...
export type CustomerProfileRouteEvalFixtureDraft = RouteEvalFixtureDraft & {
  expectedAction: "profile";
};

export function buildCustomerProfileRouteEvalFixtures(params: {
  config: HetangOpsConfig;
  now: Date;
  samples: CustomerProfileUtteranceSample[];
}): CustomerProfileRouteEvalFixtureDraft[] {
  return params.samples.map((sample) => {
    ...
    if (intent.lane !== "query" || intent.kind !== "query" || intent.action !== sample.action) {
      throw new Error(
        `Customer profile primary utterance must resolve to query:${sample.action}: ${sample.id} -> ${sample.primary} -> ${intent.lane}:${intent.kind}:${intent.action}`,
      );
    }

    if (!intent.capabilityId) {
      throw new Error(
        `Customer profile primary utterance must resolve a capability id: ${sample.id} -> ${sample.primary}`,
      );
    }

    return {
      id: `customer-profile-${sample.id}`,
