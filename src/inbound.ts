...
function resolveConceptExplainReply(text: string): string {
  ...
  if (/(世界模型|如何搭建|怎么搭建|搭建|构建|建模|架构|体系|结构|机制|框架|方法论|飞轮|业务模型|商业模型|设计)/u.test(normalized)) {
    return [
      ...
      "先定义实体：门店、顾客、技师、项目、订单、支付、充值、优惠券、排班、房间和外部渠道。",
      "再定义关系：顾客到店、技师服务、项目消耗、现金到账、储值负债、活动转化、排班供给和复购留存。",
      "最后定义状态和信号：客流、客单、钟数、点钟率、加钟率、充值、沉睡会员、反结、折扣和数据缺口。",
      "搭建顺序应该是：数据事实层 -> 指标口径层 -> capability 图谱 -> answerability 自检 -> 主动诊断和经营动作闭环。",
    ].join("\n");
  }
  if (/(AI物理模型|ai物理模型|物理模型|经营世界|服务实体|经营模型|定义)/u.test(normalized)) {
    return [
      "你问的是门店经营的物理模型。",
      "可以把它定义成：门店不是一堆散指标，而是一个由人、客、钟、钱、排班、项目和规则共同构成的经营系统。",
      "人和技师是供给，客流和转化是需求，钟数和客单价是产出，储值、折扣和复购是资金与留存，排班和规则是约束。",
      "所以回答经营问题时，不是先找门店名，而是先找这个系统里哪一层出了变化，再往下看指标和动作。",
    ].join("\n");
  }
