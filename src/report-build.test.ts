...
describe("buildDailyStoreReport refresh orchestration", () => {
  it("writes all marts first and refreshes analytics views once", async () => {
    ...
    const store = {
      ...
      listStoreExternalContextEntries: vi.fn().mockResolvedValue([]),
  ...
  it("uses previous-week daily report baseline for comparison pain signals", async () => {
    computeDailyStoreMetricsMock.mockResolvedValue({
      metrics: buildMetrics({
        serviceRevenue: 15_000,
        customerCount: 92,
        averageTicket: 163.04,
        rechargeCash: 12_000,
    ...
    const store = {
      ...
      listStoreReview7dByDateRange: vi.fn().mockResolvedValue([]),
      listStoreSummary30dByDateRange: vi.fn().mockResolvedValue([]),
      getDailyReport: vi.fn().mockResolvedValue(baselineReport),
      getStoreEnvironmentDailySnapshot: vi.fn().mockResolvedValue(null),
      listStoreExternalContextEntries: vi.fn().mockResolvedValue([]),
      saveDailyMetrics: vi.fn().mockResolvedValue(undefined),
      replaceDailyAlerts: vi.fn().mockResolvedValue(undefined),
    ...
    expect(store.getDailyReport).toHaveBeenCalledWith("1001", "2026-03-22");
    expect(report.suggestions).toEqual(
      expect.arrayContaining([expect.stringContaining("痛点雷达【营收下滑】")]),
    );
    expect(report.markdown).toContain("营收下滑");
  });
  ...
  it("renders the formal daily report in store-manager detail format", async () => {
    ...
    const store = {
      ...
      listStoreSummary30dByDateRange: vi.fn().mockResolvedValue([]),
      getStoreEnvironmentDailySnapshot: vi.fn().mockResolvedValue({
        orgId: "1001",
        bizDate: "2026-03-29",
        weekdayIndex: 0,
        weekdayLabel: "周日",
        ...
        snapshotJson: "{}",
        collectedAt: "2026-03-29T00:00:00.000Z",
    ...
    expect(report.markdown).toContain("【痛点雷达】");
    expect(report.markdown).toContain("储值压力");
    expect(report.markdown).toContain("【世界模型补充】");
    expect(report.markdown).toContain("世界模型补充判断");
    expect(report.markdown).toContain("当前世界模型主要依据：门店经营事实、环境上下文、外部情报");
    ...
    expect(report.suggestions).toEqual(
      expect.arrayContaining([expect.stringContaining("痛点雷达【储值压力】")]),
    );
