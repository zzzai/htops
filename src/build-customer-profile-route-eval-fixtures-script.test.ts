...
const scriptPath = path.join(repoRoot, "scripts/build-customer-profile-route-eval-fixtures.ts");
...
async function writeConfig(): Promise<string> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "customer-profile-route-eval-script-"));
...
describe("build-customer-profile-route-eval-fixtures script", () => {
  it("builds fixture drafts from the checked-in customer profile user utterance samples", async () => {
