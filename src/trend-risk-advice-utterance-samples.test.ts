...
type TrendRiskAdviceUtteranceSample = {
  id: string;
  category: string;
  action: "trend" | "anomaly" | "risk" | "advice";
  label: string;
  primary: string;
  similars: string[];
...
describe("trend-risk-advice-utterance-samples", () => {
  const typed = samples as TrendRiskAdviceUtteranceSample[];
  const config = buildConfig();
  const now = new Date("2026-04-14T10:00:00+08:00");

  it("covers stable trend/anomaly/risk/advice asks with unique ids and paraphrases", () => {
    expect(typed).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "store-trend-revenue-window",
          action: "trend",
        }),
        expect.objectContaining({
          id: "store-anomaly-revenue-window",
          action: "anomaly",
        }),
        expect.objectContaining({
          id: "store-risk-window",
          action: "risk",
        }),
        expect.objectContaining({
          id: "store-advice-window",
          action: "advice",
        }),
      ]),
    );
