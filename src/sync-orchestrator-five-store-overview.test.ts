...
function buildConfig(
  ...
  return resolveHetangOpsConfig({
    ...
    reporting: {
      ...
      sendReportEnabled: overrides.sendReportEnabled ?? true,
    ...
    stores: [
      {
        orgId: "1001",
        storeName: "迎宾店",
        notification: { channel: "wecom", target: "manager-yingbin", enabled: true },
      },
    ],
  });
}
...
function buildOrchestrator(params: {
  ...
  return {
    ...
    orchestrator: new HetangSyncOrchestrator({
      config: buildConfig({ sendReportEnabled: params.sendReportEnabled }),
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
        error: vi.fn(),
...
describe("HetangSyncOrchestrator five-store daily overview delivery", () => {
  it("waits for the five-store overview when daily reports are not built and audited yet", async () => {
    const sendReport = vi.fn(async () => "迎宾店: report sent");
    const fakeStore = buildFakeStore(["send-report:2026-04-22"]);
    ...
    expect(sendReport).toHaveBeenCalledTimes(0);
    expect(sendFiveStoreDailyOverview).not.toHaveBeenCalled();
    expect(lines).toContain("2026-04-22 five-store daily overview waiting - daily reports not built/audited yet");
  ...
  it("sends the five-store overview after daily reports are built and audited when single-store sends are disabled", async () => {
    const sendReport = vi.fn(async () => "迎宾店: report sent");
    const store = buildFakeStore([
      "build-report:2026-04-22",
      "audit-daily-report-window:2026-04-22",
    ]);
    const { orchestrator, sendFiveStoreDailyOverview } = buildOrchestrator({
      sendReport,
      store,
  ...
  it("still sends the five-store overview when a single-store report is skipped for shared-group protection", async () => {
    const sendReport = vi.fn(
      async () => "迎宾店: skipped - store manager notification target matches shared delivery",
    );
    const store = buildFakeStore([
      "build-report:2026-04-22",
      "audit-daily-report-window:2026-04-22",
    ...
    expect(lines).toContain(
      "迎宾店: skipped - store manager notification target matches shared delivery",
    );
    expect(lines).toContain("five-store daily overview sent for 2026-04-22");
    expect(store.markScheduledJobCompleted).toHaveBeenCalledWith(
      "send-report",
