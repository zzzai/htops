...
import type { HetangConversationReviewOverview } from "../types.js";
...
`);
  ...
  it("surfaces nightly conversation review input=0 as an explicit doctor warning", () => {
    const overview: HetangConversationReviewOverview = {
      latestRun: {
        reviewRunId: "review-2026-05-06",
        reviewDate: "2026-05-06",
        sourceWindowStart: "2026-05-05T00:00:00.000+08:00",
        sourceWindowEnd: "2026-05-06T00:00:00.000+08:00",
        status: "completed",
        inputConversationCount: 0,
        inputShadowSampleCount: 0,
        inputAnalysisJobCount: 0,
        findingCount: 0,
        createdAt: "2026-05-06T05:26:00.000+08:00",
        updatedAt: "2026-05-06T05:26:00.000+08:00",
      },
      summary: {
    ...
    expect(formatConversationReviewInputLine(overview)).toBe(
      "Conversation review latest: reviewDate=2026-05-06 | status=completed | input=0 conversations / 0 shadow / 0 analysis | findings=0",
    );
    expect(formatConversationReviewInputWarning(overview)).toBe(
      "nightly-conversation-review input=0; audit sample flow is empty for 2026-05-05T00:00:00.000+08:00 to 2026-05-06T00:00:00.000+08:00",
    );
  });
  ...
  it("renders boundary failure classes in semantic quality top failures and backlog", () => {
    const line = formatSemanticQualityLine({
      windowHours: 24,
      totalCount: 9,
      successCount: 4,
      ...
      fallbackUsedCount: 0,
      fallbackRate: 0,
      latestOccurredAt: "2026-05-02T10:00:00.000Z",
      topFailureClasses: [
        { failureClass: "missing_cost_model_contract", count: 2 },
        { failureClass: "unsupported_external_research", count: 1 },
      ],
      topAnalysisFrameworks: [],
      topRouteUpgrades: [],
      optimizationBacklog: [
        {
          ...
          count: 2,
          ownerModule: "src/semantic-answerability.ts",
          recommendedAction: "补正式成本模型 contract。",
          priority: "high",
        },
        {
          ...
          count: 1,
          ownerModule: "src/semantic-intent.ts",
          recommendedAction: "把外部研究问题分流到 HQ lane。",
          priority: "medium",
        },
      ],
      sampleCandidates: [
        {
          ...
          count: 2,
          ownerModule: "src/semantic-answerability.ts",
          sampleTag: "cost_model_boundary",
          prompt: "义乌店3月可分配现金利润多少",
        },
      ],
    });

    expect(line).toContain(
      "top_failures=missing_cost_model_contract:2,unsupported_external_research:1",
    );
    expect(line).toContain(
      "backlog=missing_cost_model_contract@src/semantic-answerability.ts(high),unsupported_external_research@src/semantic-intent.ts(medium)",
    );
    expect(line).toContain("samples=义乌店3月可分配现金利润多少");
  });
});
