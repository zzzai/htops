...
const scriptPath = path.join(repoRoot, "scripts/review-metric-utterance-coverage.ts");
...
async function writeFixtureInputs(): Promise<{
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "metric-coverage-script-"));
...
describe("review-metric-utterance-coverage script", () => {
  it("prints uncovered metric paraphrases from inbound audits", async () => {
