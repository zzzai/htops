...
import {
  ...
  HIGH_VALUE_MEMBER_ACTIVE_MAX_SILENT_DAYS,
  POTENTIAL_GROWTH_MAX_VISIT_COUNT_90D,
  POTENTIAL_GROWTH_PAY_AMOUNT_90D,
  qualifiesHighValueMemberWindow,
} from "./customer-semantics.js";
...
function resolvePrimarySegment(params: {
  ...
  if (isHighValueMember && params.daysSinceLastVisit <= HIGH_VALUE_MEMBER_ACTIVE_MAX_SILENT_DAYS) {
  ...
  if (
    params.daysSinceLastVisit <= HIGH_VALUE_MEMBER_ACTIVE_MAX_SILENT_DAYS &&
    params.payAmount90d >= POTENTIAL_GROWTH_PAY_AMOUNT_90D &&
    params.visitCount90d <= POTENTIAL_GROWTH_MAX_VISIT_COUNT_90D
  ) {
...
export function buildCustomerConversionCohorts(params: {
  ...
  for (const consumeBill of params.consumeBills) {
    ...
    const customers = resolveConsumeCustomers({
      ...
      currentMembers: params.currentMembers,
      currentMemberCards: params.currentMemberCards,
    }).filter((identity) => identity.identityStable);
    const timeMs =
