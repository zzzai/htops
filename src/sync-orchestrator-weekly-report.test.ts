...
function buildOrchestrator(params: {
  ...
  return {
    ...
    orchestrator: new HetangSyncOrchestrator({
      ...
      sendWeeklyChartImage: vi.fn(async () => "weekly chart image sent") as unknown as (params: {
        weekEndBizDate?: string;
        now?: Date;
      }) => Promise<string>,
...
describe("HetangSyncOrchestrator weekly report delivery", () => {
  it("waits for the Monday weekly report when daily reports are not fully sent", async () => {
    const sendReport = vi.fn(async () => {
      throw new Error("wecom down");
    });
    const { orchestrator, sendWeeklyReport, store } = buildOrchestrator({
      sendReport,
    });

    const lines = await orchestrator.runDueJobs(new Date("2026-04-20T02:06:00Z"));
    ...
    expect(lines).toContain("2026-04-19 weekly report waiting - daily reports not fully sent yet");
  ...
  it("sends the Monday weekly report after daily reports finish in the same run", async () => {
    ...
    expect(sendWeeklyReport).toHaveBeenCalledWith({
      ...
      now: new Date("2026-04-20T02:06:00Z"),
    });
    expect(lines).toContain("迎宾店: report sent");
    expect(lines).toContain("weekly report sent for 2026-04-19");
