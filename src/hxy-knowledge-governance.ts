...
export function classifyHxyClaimTheme(claim: string): HxyClaimTheme {
  ...
  if (/社区.*(泡脚|按摩|小店|私域)|泡脚按摩小店|社区小店/u.test(claim)) {
  ...
  if (/(\d+|\d+\.\d+).*(元|分钟)|价格|套餐|主推款|入口款|护理包|sku|spu/u.test(claim)) {
...
function groupClaimsByTheme(claims: HxyKnowledgeClaim[]): Record<HxyClaimTheme, HxyKnowledgeClaim[]> {
  ...
  for (const claim of claims) {
    const theme = classifyHxyClaimTheme(claim.claim);
    grouped[theme] ??= [];
...
function isEligibleCurrentCandidate(claim: HxyKnowledgeClaim, theme: HxyClaimTheme): boolean {
  ...
  const noise =
    /市场其他品牌调研|指标体系表|品牌数据填报页|竞品差异化矩阵|旗舰大店|优劣势|属性维度|IPO|C轮|上市|投前|收购方|平安集团|腾讯|阶段目标拆解|核心KPI|对赌协议|候选人画像|CMO|薪酬方案|基本月薪|绩效奖金|供应商Portal|税务申报|对账|发货确认/u;
  if (noise.test(text)) {
    return false;
  }
  switch (theme) {
    case "community_store_positioning":
      return /社区.*(小店|泡脚|按摩|养生|信任)|泡脚按摩小店|社区小店|私域复购/u.test(text);
    ...
    case "product_price_model":
      return (
        /主推款|招牌款|基础款|尊享款|价格|¥|(\d+|\d+\.\d+)\s*(元|分钟|min)|护理包|泡脚包/u.test(text) &&
        /泡脚|按摩|药浴|手法|护理包|套餐|草本/u.test(text)
      );
    case "store_financial_model":
      return isStoreFinancialModelCandidate(text);
    case "customer_segment":
      return /客群|人群|用户|店长|悦己|女性|年轻|社区家庭|老人|中老年/u.test(text);
    case "data_ai_model":
      return (
...
function isStoreFinancialModelCandidate(text: string): boolean {
  if (/薪酬|月薪|绩效|期权|候选人|CMO|VP|总监|资金用途|品牌影响力|产品研发|投资回报与风险控制/u.test(text)) {
    return false;
  }
  const signalCount = [
    /单店|门店/u,
...
function scoreCandidate(claim: HxyKnowledgeClaim, theme: HxyClaimTheme): number {
  let score = claim.confidence * 100 + Math.min(claim.claim.length / 10, 20);
