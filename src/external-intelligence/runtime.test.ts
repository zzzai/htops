...
describe("HetangOpsRuntime.buildExternalBriefIssue", () => {
  it("collects one issue, skips blocked and stale candidates, persists ordered items, and delivers the HQ brief", async () => {
    ...
    try {
      const issue = await (runtime as any).buildExternalBriefIssue({
