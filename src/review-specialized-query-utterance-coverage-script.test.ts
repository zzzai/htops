...
const scriptPath = path.join(repoRoot, "scripts/review-specialized-query-utterance-coverage.ts");
...
async function writeFixtureInputs(): Promise<{
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "specialized-query-coverage-script-"));
...
describe("review-specialized-query-utterance-coverage script", () => {
  it("prints uncovered specialized-query paraphrases from inbound audits", async () => {
