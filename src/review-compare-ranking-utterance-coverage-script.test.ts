...
const scriptPath = path.join(repoRoot, "scripts/review-compare-ranking-utterance-coverage.ts");
...
async function writeFixtureInputs(): Promise<{
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "compare-ranking-coverage-script-"));
...
describe("review-compare-ranking-utterance-coverage script", () => {
  it("prints uncovered compare/ranking paraphrases from inbound audits", async () => {
