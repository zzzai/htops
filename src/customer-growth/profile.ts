...
import { buildCustomerGrowthProfileInsight } from "./ai/profile-insight.js";
import { buildCustomerGrowthTagAdvisor } from "./ai/tag-advisor.js";
import type {
  CustomerGrowthProfileInsight,
  CustomerGrowthTagAdvisor,
} from "./ai/contracts.js";

export type CustomerProfileRuntime = {
...
type StructuredCustomerOperatingProfileSnapshot = {
  identity_profile?: Record<string, unknown>;
  spending_profile?: Record<string, unknown>;
  service_need_profile?: Record<string, unknown>;
  interaction_profile?: Record<string, unknown>;
  preference_profile?: Record<string, unknown>;
  scenario_profile?: Record<string, unknown>;
  relationship_profile?: Record<string, unknown>;
  opportunity_profile?: Record<string, unknown>;
};

type StructuredCustomerEvidenceItem = {
...
type StructuredCustomerEvidenceSummary = {
  facts: StructuredCustomerEvidenceItem[];
  observations: StructuredCustomerEvidenceItem[];
  inferences: StructuredCustomerEvidenceItem[];
};

export type StructuredCustomerProfileLookupResult = {
  org_id: string;
  store_name: string;
  snapshot_biz_date: string;
  matched_members: Array<{
    ...
    current_profile: {
      ...
      tech_loyalty_segment?: string;
      pay_amount_90d?: number;
      visit_count_90d?: number;
      current_stored_amount?: number;
      current_silent_days?: number;
      top_tech_name?: string;
      tags?: string[];
    } | null;
    operating_profile?: StructuredCustomerOperatingProfileSnapshot;
    evidence_summary?: StructuredCustomerEvidenceSummary;
    ai_advisory?: {
      profile_insight?: {
      ...
      tag_advisor?: {
        soft_tags?: string[];
        tag_hypotheses?: string[];
        tag_reasons?: string[];
      };
    };
  }>;
  legacy_profile_text?: string;
};

type StructuredCustomerProfileMember =
  StructuredCustomerProfileLookupResult["matched_members"][number];
type StructuredCustomerProfileInsightAdvisory = NonNullable<
  StructuredCustomerProfileMember["ai_advisory"]
>["profile_insight"];
type StructuredCustomerTagAdvisorAdvisory = NonNullable<
  StructuredCustomerProfileMember["ai_advisory"]
>["tag_advisor"];
...
function pickOperatingProfileRow(
  rows: CustomerOperatingProfileDailyRecord[],
  memberId: string,
  customerIdentityKey?: string,
): CustomerOperatingProfileDailyRecord | undefined {
  const exact = rows.find((row) => row.memberId === memberId);
  if (exact) {
    return exact;
  }
  if (customerIdentityKey) {
    return rows.find((row) => row.customerIdentityKey === customerIdentityKey);
  }
  return undefined;
}

function mapStructuredOperatingProfile(
  snapshot?: CustomerOperatingProfileDailyRecord,
): StructuredCustomerOperatingProfileSnapshot | undefined {
  if (!snapshot) {
    return undefined;
  }
  return {
    identity_profile: snapshot.identityProfileJson,
    spending_profile: snapshot.spendingProfileJson,
    service_need_profile: snapshot.serviceNeedProfileJson,
    interaction_profile: snapshot.interactionProfileJson,
    preference_profile: snapshot.preferenceProfileJson,
    scenario_profile: snapshot.scenarioProfileJson,
    relationship_profile: snapshot.relationshipProfileJson,
    opportunity_profile: snapshot.opportunityProfileJson,
  };
}

function buildSignalValueText(signal: CustomerOperatingSignalRecord): string | undefined {
  if (typeof signal.valueText === "string" && signal.valueText.trim().length > 0) {
    return signal.valueText.trim();
  }
  if (typeof signal.valueNum === "number" && Number.isFinite(signal.valueNum)) {
    return String(signal.valueNum);
  }
  return undefined;
}

function resolveEvidenceLabel(signalDomain: string, signalKey: string): string {
  const key = 
    signalDomain + "|" + signalKey;
  switch (key) {
    case "service_need|primary_need":
      return "服务诉求";
    case "interaction_style|communication_style":
      return "互动风格";
    case "contact_preference|preferred_channel":
      return "触达偏好";
    case "time_preference|preferred_daypart":
      return "时段偏好";
    case "tech_preference|preferred_tech_code":
      return "技师偏好";
    default:
      return signalDomain + "/" + signalKey;
  }
}

function resolveObservationSourceLabel(
  observation: CustomerServiceObservationRecord,
): string | undefined {
  if (observation.sourceType === "self_reported") {
    return "顾客自述";
  }
  if (observation.sourceType === "system_fact") {
    return "系统事实";
  }
  if (observation.sourceType === "system_inferred") {
    return "系统推断";
  }
  switch (observation.sourceRole) {
    case "technician":
      return "技师观察";
    case "front_desk":
      return "前台观察";
...
function resolveEvidenceDisplayText(signal: CustomerOperatingSignalRecord): string {
  const label = resolveEvidenceLabel(signal.signalDomain, signal.signalKey);
  const valueText = buildSignalValueText(signal);
  if (!valueText) {
    return label;
  }
  const signalIdentity = signal.signalDomain + "|" + signal.signalKey;
  if (signal.truthBoundary === "hard_fact") {
    return label + "：" + valueText;
  }
  if (signal.truthBoundary === "observed_fact") {
    return label + "：" + (signal.confidence === "high" ? "观察到" : "观察偏向") + valueText;
  }
  if (signalIdentity === "contact_preference|preferred_channel") {
    return label + "：可能更接受" + valueText;
  }
  return label + "：可能" + valueText;
}

function buildStructuredEvidenceSummary(params: {
  signals: CustomerOperatingSignalRecord[];
  observations: CustomerServiceObservationRecord[];
}): StructuredCustomerEvidenceSummary | undefined {
  const observationById = new Map(
    params.observations.map((observation) => [observation.observationId, observation]),
  );
  const summary: StructuredCustomerEvidenceSummary = {
    facts: [],
    observations: [],
    inferences: [],
  };

  for (const signal of [...params.signals].sort((left, right) => left.signalId.localeCompare(right.signalId))) {
    const matchedObservations = signal.sourceObservationIds
      .map((observationId) => observationById.get(observationId))
      .filter((observation): observation is CustomerServiceObservationRecord => Boolean(observation));
    const sourceSummary = uniqueStringList(
      matchedObservations.map((observation) => resolveObservationSourceLabel(observation)),
    );
    const item: StructuredCustomerEvidenceItem = {
      signal_domain: signal.signalDomain,
      signal_key: signal.signalKey,
      value_text: buildSignalValueText(signal),
      confidence: signal.confidence,
      truth_boundary: signal.truthBoundary,
      display_text: resolveEvidenceDisplayText(signal),
      source_summary:
        sourceSummary.length > 0
          ? sourceSummary
          : signal.truthBoundary === "predicted_signal" || signal.truthBoundary === "inferred_label"
            ? ["系统推断"]
            : ["经营快照"],
    };
    if (signal.truthBoundary === "hard_fact") {
      summary.facts.push(item);
    } else if (signal.truthBoundary === "observed_fact") {
      summary.observations.push(item);
    } else {
      summary.inferences.push(item);
    }
  }

  return summary.facts.length > 0 || summary.observations.length > 0 || summary.inferences.length > 0
    ? summary
    : undefined;
}

function readStructuredProfileText(
  profile: StructuredCustomerOperatingProfileSnapshot | undefined,
  section: keyof StructuredCustomerOperatingProfileSnapshot,
  key: string,
): string | undefined {
  const sectionValue = profile?.[section];
  const value = asObject(sectionValue)[key];
  return typeof value === "string" && value.trim().length > 0 ? value.trim() : undefined;
}

function buildOperatingProfileSupplementLines(
  profile: StructuredCustomerOperatingProfileSnapshot | undefined,
): string[] {
  if (!profile) {
    return [];
  ...
  const lines: string[] = [];
  const primaryNeed = readStructuredProfileText(profile, "service_need_profile", "primary_need");
  const communicationStyle = readStructuredProfileText(
    profile,
    "interaction_profile",
    "communication_style",
  );
  const preferredDaypart = readStructuredProfileText(
    profile,
    "preference_profile",
    "preferred_daypart",
  );
  const preferredTech =
    readStructuredProfileText(profile, "preference_profile", "preferred_tech_name") ??
    readStructuredProfileText(profile, "relationship_profile", "top_tech_name");
  const opportunityScore = asObject(profile.opportunity_profile).reactivation_priority_score;

  if (primaryNeed) {
    lines.push("服务诉求：" + primaryNeed);
  }
  if (communicationStyle) {
    lines.push("互动风格：" + communicationStyle);
  }
  if (preferredDaypart) {
    lines.push("偏好时段：" + preferredDaypart);
  }
  if (preferredTech) {
    lines.push("关系偏好：" + preferredTech);
  }
  if (typeof opportunityScore === "number" && Number.isFinite(opportunityScore)) {
    lines.push("经营机会：当前唤回优先分 " + String(round(opportunityScore, 0)));
  }
  return lines;
}

function buildEvidenceBoundaryGroupLine(
  title: string,
  items: StructuredCustomerEvidenceItem[],
): string | undefined {
  if (items.length === 0) {
    return undefined;
  }
  const rendered = items.slice(0, 3).map((item) => {
    const sourceText = item.source_summary.length > 0 ? "（来源：" + item.source_summary.join("、") + "）" : "";
    return item.display_text + sourceText;
  });
  return title + "：" + rendered.join("；");
}

function buildEvidenceBoundaryLines(
  summary: StructuredCustomerEvidenceSummary | undefined,
): string[] {
  if (!summary) {
    return [];
  }
  return [
    buildEvidenceBoundaryGroupLine("事实线索", summary.facts),
    buildEvidenceBoundaryGroupLine("观察线索", summary.observations),
    buildEvidenceBoundaryGroupLine("推断线索", summary.inferences),
...
function mapStructuredCustomerProfile(params: {
  member: MemberCurrentRecord & Record<string, unknown>;
  profile?: CustomerProfile90dRow;
  operatingProfile?: CustomerOperatingProfileDailyRecord;
  evidenceSummary?: StructuredCustomerEvidenceSummary;
}): StructuredCustomerProfileLookupResult["matched_members"][number] {
  const { member, profile, operatingProfile, evidenceSummary } = params;
  return {
    member_id: member.memberId,
    customer_name: member.name,
    phone_suffix: lastPhoneDigits(member.phone),
    member_level_name: typeof member.memberLevelName === "string" ? member.memberLevelName : undefined,
    birthday: typeof member.birthday === "string" ? member.birthday : undefined,
    current_member_state: {
    ...
    current_profile: profile
      ...
      : null,
    operating_profile: mapStructuredOperatingProfile(operatingProfile),
    evidence_summary: evidenceSummary,
  };
}

function mapStructuredProfileInsightAdvisory(
  insight: CustomerGrowthProfileInsight | null,
): StructuredCustomerProfileInsightAdvisory {
  if (!insight) {
    return undefined;
  }
  return {
    profile_narrative: insight.profileNarrative,
    high_value_signals: insight.highValueSignals,
    risk_signals: insight.riskSignals,
    missing_facts: insight.missingFacts,
  };
}

function mapStructuredTagAdvisorAdvisory(
  advisor: CustomerGrowthTagAdvisor | null,
): StructuredCustomerTagAdvisorAdvisory {
  if (!advisor) {
    return undefined;
  }
  return {
    soft_tags: advisor.softTags,
    tag_hypotheses: advisor.tagHypotheses,
    tag_reasons: advisor.tagReasons,
  };
}

async function buildStructuredCustomerProfileAiAdvisory(params: {
  config: HetangOpsConfig;
  orgId: string;
  storeName: string;
  bizDate: string;
  member: StructuredCustomerProfileLookupResult["matched_members"][number];
}): Promise<StructuredCustomerProfileLookupResult["matched_members"][number]["ai_advisory"] | undefined> {
  const facts = {
    orgId: params.orgId,
    storeName: params.storeName,
    snapshotBizDate: params.bizDate,
    member: params.member,
  };
  const [profileInsight, tagAdvisor] = await Promise.all([
    buildCustomerGrowthProfileInsight({
      config: params.config,
      facts,
    }),
    buildCustomerGrowthTagAdvisor({
      config: params.config,
      facts,
    }),
  ]);
  const advisory = {
    profile_insight: mapStructuredProfileInsightAdvisory(profileInsight),
    tag_advisor: mapStructuredTagAdvisorAdvisory(tagAdvisor),
  };
  if (!advisory.profile_insight && !advisory.tag_advisor) {
    return undefined;
  }
  return advisory;
}

export async function lookupStructuredCustomerProfile(params: {
  runtime: CustomerProfileRuntime;
  config: HetangOpsConfig;
  orgId: string;
  bizDate: string;
  ...
  memberId?: string;
  now?: Date;
}): Promise<StructuredCustomerProfileLookupResult | null> {
  const phoneSuffix = params.phoneSuffix?.trim();
  ...
  const [profileRows, operatingProfileRows] = await Promise.all([
    params.runtime.listCustomerProfile90dByDateRange
      ? params.runtime.listCustomerProfile90dByDateRange({
          orgId: params.orgId,
          startBizDate: shiftBizDate(params.bizDate, -89),
          endBizDate: params.bizDate,
        })
      : Promise.resolve([]),
    params.runtime.listCustomerOperatingProfilesDaily
      ? params.runtime.listCustomerOperatingProfilesDaily({
          orgId: params.orgId,
          bizDate: params.bizDate,
        })
  ...
  const matchedMembers = await Promise.all(
    members.map(async (member) => {
      const profile = pickProfileRow(profileRows, member.memberId, params.bizDate);
      const customerIdentityKey =
        profile?.customerIdentityKey ??
        (member.memberId ? "member:" + member.memberId : undefined);
      const [operatingSignals, customerServiceObservations] = await Promise.all([
        params.runtime.listCustomerOperatingSignals
          ? params.runtime.listCustomerOperatingSignals({
              orgId: params.orgId,
          ...
          : Promise.resolve([]),
        params.runtime.listCustomerServiceObservations
          ? params.runtime.listCustomerServiceObservations({
              orgId: params.orgId,
              memberId: member.memberId,
              customerIdentityKey,
      ...
      const mappedMember = mapStructuredCustomerProfile({
        member: member as MemberCurrentRecord & Record<string, unknown>,
        profile,
        operatingProfile: pickOperatingProfileRow(
          operatingProfileRows,
          member.memberId,
          customerIdentityKey,
        ),
        evidenceSummary: buildStructuredEvidenceSummary({
          signals: operatingSignals,
          observations: customerServiceObservations,
        }),
      });
      const aiAdvisory = await buildStructuredCustomerProfileAiAdvisory({
        config: params.config,
        orgId: params.orgId,
        storeName,
        bizDate: params.bizDate,
      ...
      return aiAdvisory ? { ...mappedMember, ai_advisory: aiAdvisory } : mappedMember;
    }),
  );
  ...
  return {
    ...
    snapshot_biz_date: params.bizDate,
...
export async function executePhoneSuffixCustomerProfileQuery(params: {
  ...
  const customerIdentityKey =
    profileSnapshot?.customerIdentityKey ??
    (member.memberId ? "member:" + member.memberId : undefined);
  const [operatingProfileRows, operatingSignals, customerServiceObservations] = await Promise.all([
    params.runtime.listCustomerOperatingProfilesDaily
      ? params.runtime.listCustomerOperatingProfilesDaily({
          orgId,
          bizDate: resolvedSnapshotBizDate,
        })
      : Promise.resolve([]),
    params.runtime.listCustomerOperatingSignals
      ? params.runtime.listCustomerOperatingSignals({
  ...
  const operatingProfileSnapshot = mapStructuredOperatingProfile(
    pickOperatingProfileRow(operatingProfileRows, member.memberId, customerIdentityKey),
  );
  const evidenceSummary = buildStructuredEvidenceSummary({
    signals: operatingSignals,
    observations: customerServiceObservations,
  });
  ...
  appendSection(lines, "经营画像补充", buildOperatingProfileSupplementLines(operatingProfileSnapshot));
  appendSection(lines, "证据边界", buildEvidenceBoundaryLines(evidenceSummary));
