...
import { buildCustomerGrowthStrategyAdvisor } from "./ai/strategy-advisor.js";
import type { CustomerGrowthStrategyAdvisor } from "./ai/contracts.js";
...
export type StructuredMemberRecallLookupResult = {
  org_id: string;
  store_name: string;
  ...
  candidates: Array<{
    ...
    current_stored_balance_inferred: number;
    projected_balance_days_left: number | null;
    ...
    time_pattern: {
      ...
      preferred_daypart_share_90d: number | null;
      dominant_weekday: string | null;
      preferred_weekday_share_90d: number | null;
      late_night_visit_share_90d: number | null;
      overnight_visit_share_90d: number | null;
    } | null;
    strategy: {
      churn_risk_label: string;
      revisit_probability_7d: number;
      revisit_window_label: string;
      lifecycle_momentum_label: string;
    } | null;
    birthday: {
      next_birthday_biz_date: string | null | undefined;
      birthday_window_days: number | null | undefined;
    };
    top_tech_name: string | null | undefined;
    ai_advisory?: {
      strategy_advisor?: {
        contact_angle?: string;
        talking_points?: string[];
        offer_guardrails?: string[];
        do_not_push_flags?: string[];
      };
    };
  }>;
};
...
type SegmentDefinition = {
  key: CustomerPrimarySegment | "recharge-not-visited-member";
  label: string;
  aliases: RegExp[];
};
...
const SEGMENT_DEFINITIONS: SegmentDefinition[] = [
  ...
  },
  {
    ...
    label: "沉睡会员",
    aliases: [/(?:沉睡|睡眠|沉默)(?:会员|客户|顾客)/u, /很久没来(?:了)?(?:的)?(?:会员|客户|顾客|客人)?/u],
  },
  {
    key: "recharge-not-visited-member",
    label: "充值未到店会员",
    aliases: [
      /(谁充了钱还没来过|充了钱还没来过的客人有哪些|充了钱还没来过的会员有哪些|充值了还没来过|充卡了还没来过|充了值还没来过)/u,
      /(充了钱.*没来过|充值了.*没来过|充卡了.*没来过)/u,
    ],
  },
  {
...
function diffBizDays(laterBizDate: string, earlierBizDate: string): number {
  const later = Date.parse(`${laterBizDate}T00:00:00Z`);
  const earlier = Date.parse(`${earlierBizDate}T00:00:00Z`);
...
type RechargeWithoutVisitRow = {
  memberId: string;
  customerName: string;
  phoneSuffix?: string;
  latestRechargeBizDate: string;
  rechargeTotal: number;
  rechargeCount: number;
  currentStoredBalance: number;
  daysSinceLatestRecharge: number;
};
...
export function resolveServingCustomerSegmentListMatch(
  ...
  if (segment.key === "recharge-not-visited-member") {
    return null;
  }
  if (
    asksCount(text) ||
...
export function hasRuntimeOnlyCustomerSegmentMatch(text: string): boolean {
  return resolveCustomerSegmentMatch(text)?.key === "recharge-not-visited-member";
}
...
function resolveRechargeMemberId(
  ...
  const explicitMemberId = String(
    parsed.MemberId ?? parsed.MemberID ?? parsed.CustomerId ?? parsed.CId ?? "",
  ).trim();
  if (explicitMemberId) {
  ...
  const phone = normalizePhone(
    String(parsed.MemberPhone ?? parsed.Phone ?? parsed.Mobile ?? parsed.CPhone ?? "").trim(),
  );
  if (phone) {
    const matched = currentMembers.find((member) => normalizePhone(member.phone) === phone);
    if (matched) {
      return matched.memberId;
    }
  }

  const displayName = String(parsed.MemberName ?? parsed.Name ?? parsed.CName ?? "").trim();
  if (displayName) {
    const matched = currentMembers.find(
      (member) => normalizeText(member.name) === normalizeText(displayName),
    );
    if (matched) {
      return matched.memberId;
    }
...
async function loadRechargeWithoutVisitSnapshot(params: {
  runtime: CustomerQueryRuntime;
  orgId: string;
  frame: HetangQueryTimeFrame;
}): Promise<{ bizDate: string; rows: RechargeWithoutVisitRow[] } | null> {
  if (!params.runtime.listCurrentMembers || !params.runtime.listRechargeBillsByDateRange) {
    return null;
  }

  const snapshotBizDate =
    params.frame.kind === "single" ? params.frame.bizDate : params.frame.endBizDate;
  const rechargeStartBizDate =
    params.frame.kind === "range" ? params.frame.startBizDate : shiftBizDate(snapshotBizDate, -365);

  const [currentMembers, rechargeBills] = await Promise.all([
    params.runtime.listCurrentMembers({ orgId: params.orgId }),
    params.runtime.listRechargeBillsByDateRange({
      orgId: params.orgId,
      startBizDate: rechargeStartBizDate,
      endBizDate: snapshotBizDate,
    }),
  ]);

  const currentMemberById = new Map(currentMembers.map((member) => [member.memberId, member] as const));
  const aggregate = new Map<string, RechargeWithoutVisitRow>();

  for (const rechargeBill of rechargeBills) {
    if (rechargeBill.antiFlag) {
      continue;
    }
    const memberId = resolveRechargeMemberId(rechargeBill, currentMembers);
    if (!memberId) {
      continue;
    }
    const member = currentMemberById.get(memberId);
    if (!member || member.lastConsumeTime) {
      continue;
    }
    const previous = aggregate.get(memberId);
    const latestRechargeBizDate =
      previous && previous.latestRechargeBizDate > rechargeBill.bizDate
        ? previous.latestRechargeBizDate
        : rechargeBill.bizDate;
    aggregate.set(memberId, {
      memberId,
      customerName: member.name,
      phoneSuffix: member.phone ? member.phone.slice(-4) : undefined,
      latestRechargeBizDate,
      rechargeTotal: round((previous?.rechargeTotal ?? 0) + rechargeBill.totalAmount),
      rechargeCount: (previous?.rechargeCount ?? 0) + 1,
      currentStoredBalance: round(member.storedAmount),
      daysSinceLatestRecharge: diffBizDays(snapshotBizDate, latestRechargeBizDate),
    });
  }

  const rows = Array.from(aggregate.values())
    .filter((row) => row.daysSinceLatestRecharge >= 7)
    .sort(
      (left, right) =>
        right.rechargeTotal - left.rechargeTotal ||
        right.currentStoredBalance - left.currentStoredBalance ||
        left.customerName.localeCompare(right.customerName),
    );

  return { bizDate: snapshotBizDate, rows };
}

function renderRechargeWithoutVisitCount(params: {
  storeName: string;
  snapshotBizDate: string;
  rows: RechargeWithoutVisitRow[];
}): string {
  return [
    `${params.storeName} ${params.snapshotBizDate} 充值未到店会员 ${params.rows.length} 人（按当前已同步历史）`,
    `- 累计充值总额: ${formatCurrency(params.rows.reduce((sum, row) => sum + row.rechargeTotal, 0))}`,
    `- 当前储值余额: ${formatCurrency(params.rows.reduce((sum, row) => sum + row.currentStoredBalance, 0))}`,
  ].join("\n");
}

function renderRechargeWithoutVisitList(params: {
  storeName: string;
  snapshotBizDate: string;
  rows: RechargeWithoutVisitRow[];
}): string {
  const lines = [
    `${params.storeName} ${params.snapshotBizDate} 充值未到店会员名单（按当前已同步历史，共 ${params.rows.length} 人）`,
  ];
  for (const [index, row] of params.rows.entries()) {
    lines.push(
      `${index + 1}. ${row.customerName}${row.phoneSuffix ? ` | 尾号${row.phoneSuffix}` : ""} | 最近充值 ${row.latestRechargeBizDate} | 累计充值 ${formatCurrency(row.rechargeTotal)} | 当前余额 ${formatCurrency(row.currentStoredBalance)}`,
    );
  }
  return lines.join("\n");
}
...
function mapStructuredRecallCandidate(params: {
...
function mapStructuredStrategyAdvisor(
  advisor: CustomerGrowthStrategyAdvisor | null,
): StructuredMemberRecallLookupResult["candidates"][number]["ai_advisory"] | undefined {
  if (!advisor) {
    return undefined;
  }
  return {
    strategy_advisor: {
      contact_angle: advisor.contactAngle,
      talking_points: advisor.talkingPoints,
      offer_guardrails: advisor.offerGuardrails,
      do_not_push_flags: advisor.doNotPushFlags,
    },
  };
}

export async function lookupStructuredMemberRecallCandidates(params: {
  runtime: CustomerQueryRuntime;
  config: HetangOpsConfig;
  orgId: string;
  bizDate: string;
  limit: number;
}): Promise<StructuredMemberRecallLookupResult> {
  const queueRows = params.runtime.listMemberReactivationQueue
    ? await params.runtime.listMemberReactivationQueue({
        orgId: params.orgId,
        bizDate: params.bizDate,
      })
    : [];
  const [featureRows, strategyRows] = await Promise.all([
  ...
  const candidates = await Promise.all(
    queueRows.slice(0, params.limit).map(async (queue) => {
      const feature = featureByMemberId.get(queue.memberId);
      const strategy = strategyByMemberId.get(queue.memberId);
      const mappedCandidate = mapStructuredRecallCandidate({
        queue,
        feature,
        strategy,
      });
      const aiAdvisory = mapStructuredStrategyAdvisor(
        await buildCustomerGrowthStrategyAdvisor({
          config: params.config,
          facts: {
            orgId: params.orgId,
            storeName: getStoreName(params.config, params.orgId),
            snapshotBizDate: params.bizDate,
            candidate: mappedCandidate,
          },
        }),
      );
      return aiAdvisory ? { ...mappedCandidate, ai_advisory: aiAdvisory } : mappedCandidate;
    }),
  );

  return {
    org_id: params.orgId,
    store_name: getStoreName(params.config, params.orgId),
...
export async function executeCustomerQuery(params: {
  ...
  if (params.intent.kind === "customer_segment") {
    ...
    if (segment?.key === "recharge-not-visited-member") {
      const rechargeSnapshot = await loadRechargeWithoutVisitSnapshot({
        runtime: params.runtime,
        orgId,
        frame: params.intent.timeFrame,
      });
      if (!rechargeSnapshot) {
        return "当前环境还未接通充值未到店会员查询能力。";
      }
      if (rechargeSnapshot.rows.length === 0) {
        return `${storeName} ${rechargeSnapshot.bizDate} 按当前已同步历史，未发现充值后仍未到店的会员。`;
      }
      if (asksCount(params.intent.rawText)) {
        return renderRechargeWithoutVisitCount({
          storeName,
          snapshotBizDate: rechargeSnapshot.bizDate,
          rows: rechargeSnapshot.rows,
        });
      }
      return renderRechargeWithoutVisitList({
        storeName,
        snapshotBizDate: rechargeSnapshot.bizDate,
        rows: rechargeSnapshot.rows,
      });
