...
`htops` 不是一个单点报表脚本，也不是一个只会回复问题的聊天机器人。它是一个围绕经营事实构建的本地运营内核，负责把上游业务事实同步到本地 PostgreSQL，再在本地完成经营控制、执行闭环与协同输出。

它的核心价值不在于“出日报”，而在于：

- 把上游经营事实沉淀成本地、可追溯、可验证的事实层
- 在本地形成同步、调度、执行、反馈的闭环控制面
- 在硬事实之上提供报表、问答、推送、HQ 总览、顾客经营等不同输出面
- 在需要的环节引入 bounded AI 框架，增强语义理解、能力选择、异步分析和弱信号解释

对外可以把它理解成：

`上游经营事实 -> 本地经营事实层 -> 经营控制与执行闭环 -> 输出与协同界面`

其中，报表只是输出面的一种，不是系统本体。
...
1. 先把经营事实稳住
2. 再把控制与执行闭环稳住
3. 最后才通过不同输出面把结果交付给 HQ、店长、企微群或外部服务
